...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/seedance/deploy.sh [options]
...
Options:
  ...
  --restart-systemd <name>  Restart systemd service (e.g. openwebui-seedance).
...
if [[ -n "${RESTART_SYSTEMD}" ]]; then
  echo "[STEP] restart systemd: ${RESTART_SYSTEMD}"
  ...
  echo "[NEXT] manual start command:"
  echo "  HOST=${HOST} PORT=${PORT} bash scripts/seedance/run_openwebui.sh"
fi
...
echo "[DONE] deploy flow finished"
